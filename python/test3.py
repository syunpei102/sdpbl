...
class MainApp(tk.Frame):
    ...
    def load_settings(self):
        ...
        self.bh_start = bh.get('start', 9)
        self.bh_end = bh.get('end', 22)
        self.TIME_CHOICES = []
    ...
    def create_widgets(self):
        ...
        for d in ['日','月','火','水','木','金','土']:
            tk.Label(wk, text=d, width=4).pack(side=tk.LEFT)
        self.rows = []
        for _ in range(6):
    ...
    def open_admin(self):
        ...
        dlg.grab_set()
        bh = self.config.get('business_hours', DEFAULT_CONFIG['business_hours'])
        frame_bh = tk.LabelFrame(dlg, text="営業時間設定(時)")
        ...
        frame_st = tk.LabelFrame(dlg, text="役職(持ち場)設定")
        ...
        frame_sf = tk.LabelFrame(dlg, text="スタッフ設定")
    ...
    def open_day(self, day):
        ...
        def confirm():
            ...
            if not sel: messagebox.showwarning("エラー", "スタッフ選択してください"); return
    ...
    def create_shift(self, date, entries, dlg):
        sched = []
        ...
        json.dump(self.history, open(HISTORY_FILE, 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
        dlg.destroy()
        res = tk.Toplevel(self.master); res.title(date + " シフト一覧")
        ...
        for j, ts in enumerate(timeslots, 1): tk.Label(res, text=ts, borderwidth=1, relief="solid").grid(row=0, column=j, sticky="nsew")
        staff_list = list({rec['staff'] for rec in sched})
        for i, stf in enumerate(staff_list, 1):
            tk.Label(res, text=stf, borderwidth=1, relief="solid").grid(row=i, column=0, sticky="nsew")
            for j, ts in enumerate(timeslots, 1):
                bs, be = ts.split('-'); val = ""
                for rec in sched:
        ...
        tk.Button(res, text="閉じる", command=res.destroy).grid(row=len(staff_list) + 1, column=0, columnspan=len(timeslots) + 1, pady=5)

if __name__ == '__main__':
    root = tk.Tk(); MainApp(root); root.mainloop()